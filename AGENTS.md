...
- **还没做**：① 其余 13 个 Go agent 机械铺开（每个 `main()`→`runStdioAgent()` + `ohos_ncp.go`，共用 shim；`duckdb`/`tdengine` 是 Rust agent 走 cdylib）；② 前端 dist 重建以显示"内置"标签（可选）；③ JRE 内嵌 JVM；④ ~~`compressNativeLibs` 控体积~~ **已完成（2026-09-18）**：HAP 89.5→**49.0MB**，见「产物 / 构建 → HAP 内 native 库压缩」；此后每个 Go agent 进包只 +约 5.2MB。仍可做的只剩 feature HAP / 按需下发。
