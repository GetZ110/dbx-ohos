...
- **版本**：`AppScope/app.json5` = `versionName 1.3.4 / versionCode 1003004`，**已于 2026-09-18 发布到 GitHub release**（tag `v1.3.4-dbx0.6.9`，Latest；新增 **Oracle 内置驱动 / native child process**）。上一版 `1.3.3 / 1003003`（tag `v1.3.3-dbx0.6.9`，资产未被覆盖）。上游仍是 dbx v0.6.9；本次重建了 `libdbx_ohos.so` 并新增 `libdbx_agent_oracle.so`（HAP 69→90MB）。**随后启用 HAP 内 native 库压缩**（`ohos.pack.compressLevel`），资产已就地替换为 **48,964,907 B**（≈49MB，-45%），详见「产物 / 构建 → HAP 内 native 库压缩」。
- **未发布的工作区状态（2026-09-18 晚）**：已把**全部 16 个原生 agent 打进 HAP**（14 Go + 2 Rust），`entry/libs/arm64-v8a/` 306MB → **signed HAP 101,678,839 B / unsigned 101,587,355 B**（`versionCode` 仍是 1003004，**未改写版本号、未发 release**）。真机复验：bundled=16、15 个 agent 路径驱动全部 spawn ok、duckdb sidecar 连接成功、暖启动冒烟 12/12。详见「P0' agent 类驱动」一节。是否发下一个版本、以及这 ~290MB 新 `.so` 要不要进 git，见下。
...
- **还没做**：① ~~其余 13 个 Go agent 机械铺开~~ **已完成（2026-09-18，16 个原生 agent 全内置）**；② 前端 dist 重建以显示"内置"标签（可选）；③ JRE 内嵌 JVM；④ ~~`compressNativeLibs` 控体积~~ **已完成（2026-09-18）**：HAP 89.5→**49.0MB**，见「产物 / 构建 → HAP 内 native 库压缩」；此后每个 Go agent 进包只 +约 5.2MB（16 个全内置后 HAP ≈ **128MB**）。仍可做的只剩 feature HAP / 按需下发。
...
- ⚠️ **16 个 agent `.so` 目前还没提交（未定）**：`entry/libs/arm64-v8a/` 现在 306MB（`libdbx_agent_*.so` 14 个 Go + `libdbx_agent_tdengine.so`/`libdbx_agent_duckdb.so` + `libc++_shared.so`）。`libdbx_agent_oracle.so` 与 `libdbx_ohos.so` 是已跟踪的历史提交，其余都是新文件。**全提交会让仓库涨 ~290MB**；另一条路是把它们加进 `.gitignore`（`harmony/tools/build_all_agents.sh` 可一键重建，但要求 clone 方装齐 Go/Rust/OHOS NDK）。**这个选择没做，等定。**
...
- **tag 与 release 一起建**：`gh release create <tag> --target main --latest`（本次 `v1.3.4-dbx0.6.9` → 版本提交 `ecb64bf`，资产 `DBX_HarmonyOS_v1.3.4_dbx0.6.9_unsigned.hap` 48,964,907 B / SHA-256 `ae506d78…b4c02`）；建完在父仓库 `git fetch --tags origin` 把 tag 同步到本地，并用 `gh release view --json assets` 核对线上 digest 与本地 `sha256sum` 一致。
