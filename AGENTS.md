...
- **版本**：`AppScope/app.json5` = **`versionName 1.4.0 / versionCode 1004000`**（1.4.0 已发布，见下）。上一版 `1.3.4 / 1003004`（tag `v1.3.4-dbx0.6.9`，资产未被覆盖）：重建了 `libdbx_ohos.so` 并新增 `libdbx_agent_oracle.so`（HAP 69→90MB），随后启用 HAP 内 native 库压缩（`ohos.pack.compressLevel`）降到 **48,964,907 B**（≈49MB，-45%），详见「产物 / 构建 → HAP 内 native 库压缩」。上游仍是 dbx v0.6.9。
